  ...
  for (l = lstart; l < L; l++) {
    ...
    if(l != lstart) {
      ...
      if(gain - n_safe_reject[l - 1] * (l - l_prev) > update_thresh * p && l != L - 1) { // Update EDPP if not discarding enough
        ...
        // Reapply EDPP
        ddpp_screen(discard_beta, n, p, Xty, Xtr, Xtyhat, colinear, beta_norm, yhat_norm2,
                    ytyhat, y_norm2, lambda[l], lambda[l_prev], m, alpha, col_idx, false);
        n_safe_reject[l] = sum(discard_beta, p);
        gain = n_safe_reject[l];
      } else {
        // Apply EDPP to discard features
        if(basic) { // Apply BEDPP check
          ddpp_screen(discard_beta, n, p, Xty, Xtr, Xtyhat, colinear, 0, yhat_norm2,
                      ytyhat, y_norm2, lambda[l], lambda_max, m, alpha, col_idx, true);
        } else { // Apply EDPP check
          ddpp_screen(discard_beta, n, p, Xty, Xtr, Xtyhat, colinear, beta_norm, yhat_norm2,
                      ytyhat, y_norm2, lambda[l], lambda[l_prev], m, alpha, col_idx, false);
        }
      ...
      ddpp_init(xMat, Xtyhat, colinear, sxty, xmax_idx, lambda_max, alpha,
                row_idx, col_idx, center, scale, n, p);
